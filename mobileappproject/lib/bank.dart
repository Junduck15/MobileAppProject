    ...
    userRef.get().then((snapshot) {
      snapshot.docs.forEach((doc) {
       ...
       for(int i =0; i<probIDs.length;i++){
          String title = "";
    firestore.collection("users").doc(probIDs[i]).get().then((DocumentSnapshot ds){
      ...
      userRef.doc(probIDs[i]).collection(each[j].trim()).get().then((snapshot){
      snapshot.docs.forEach((doc){
        inProbIDs.add(doc.id);
        //print(doc.id);
      });
      for(int u = 0; u < inProbIDs.length;u++){
    firestore.collection("users").doc(probIDs[i]).collection(each[j].trim()).doc(inProbIDs[u]).get().then((DocumentSnapshot ls){
